...
<div class="container mx-auto px-6 py-20">
  ...
  @if(count($cart) > 0)
        ...
        @foreach($cart as $id => $item)
          ...
          <tr class="border-b border-[#e3e3e0] hover:bg-[#f9f7f5] transition">
            ...
            <td class="p-3 flex items-center">
              <img src="{{ asset('storage/' . $item['image']) }}" class="w-16 h-16 rounded-lg mr-3 object-cover">
              {{ $item['name'] }}
            </td>
            ...
            <!-- Quantity -->
            <td class="p-3 text-center">
              <!-- ====== IMPORTANT: single form contains input + update button ====== -->
              <form action="{{ route('user.cart.update', $id) }}" method="POST" class="flex items-center justify-center gap-2">
                @csrf
                <input type="number" name="quantity" value="{{ $item['quantity'] }}" min="1"
                      class="w-16 border border-[#ccc] rounded px-2 py-1 text-center focus:outline-none focus:ring-1 focus:ring-[#a58c63]">
                <button type="submit" 
                        class="bg-[#a58c63] text-white px-3 py-1 rounded hover:bg-[#8d7753] transition">
                  Update
                </button>
              </form>
            </td>
            ...
            <!-- Total -->
            <td class="p-3 font-semibold">₱{{ number_format($itemTotal, 2) }}</td>
          </tr>
        @endforeach
      </tbody>
    ...
    <div class="mt-6 flex justify-between items-center">
      ...
      <div class="flex gap-3">
        <!-- Clear Cart -->
        <form action="{{ route('user.cart.clear') }}" method="POST" onsubmit="return confirm('Clear all items?');">
          @csrf
  ...
  @else
    <p class="text-gray-600 mt-6">Your cart is empty.</p>
  @endif
</div>
